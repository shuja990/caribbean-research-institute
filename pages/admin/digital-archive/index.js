...
const NewsAdmin = (props) => {
    ...
    const handleDelete = async (id) => {
        try {
            setLoading(true);
            const documentRef = doc(db, "digitalArchive", id);
                    ...
                    <Row className="align-items-center w-100">
                        ...
                        <Col className="d-flex justify-content-end">
                            <Button
                                ...
                                variant="light"
                                onClick={() => router.push("/admin/digital-archive/create")}
                            >
                                Add Digital Archive Item
                            </Button>
                        </Col>
                    </Row>
                    {loading ? (
                                ...
                                <tbody>
                                    {products.map((product) => (
                                        <tr key={product?.id}>
                                            ...
                                            <td>{product?.title}</td>
                                            <td>{product?.file}</td>
                                            <td>{product?.tags?.map(tag => <div className="bg-green white rounded border-black">{tag}</div>)}</td>
                                            <td>
                                                <Button
                                                    variant="light"
